...
import { useEffect, useState } from 'react'; // Necesario para el estado del menú móvil y useEffect
...
// Componente de Icono para Beneficios (Sin cambios)
...
// Componente de Tarjeta de Característica (Sin cambios)
...
export default function Home() {
  ...
  useEffect(() => {
    // Solo actuar después de que la carga inicial de autenticación haya terminado
    if (!authLoading) {
      if (user) {
        ...
        // Si el usuario no está autenticado, asegurarse de que se muestre la landing page
        console.log("[HomePage] Usuario no autenticado, mostrando landing page.");
      }
    }
  }, [user, authLoading, router]);

  // 1. Mostrar un loader mientras se verifica el estado de autenticación
  if (authLoading) {
    return (
      <div className="flex min-h-screen items-center justify-center bg-gradient-to-br from-teal-50 via-white to-emerald-50 font-sans">
        <div className="flex flex-col items-center">
          ...
          <p className="text-xl font-semibold text-teal-700">Cargando SOFIA AI...</p>
        </div>
      </div>
    );
  }

  // 2. Si no está cargando y hay un usuario, el useEffect ya está manejando la redirección.
  // Devolver un loader o null aquí para evitar un flash de contenido de la landing page.
  if (user) { // No necesitamos !authLoading aquí porque ya se manejó arriba
    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-gradient-to-br from-teal-50 via-white to-emerald-50 font-sans">
         <div className="flex flex-col items-center">
          ...
          <p className="text-xl font-semibold text-teal-700">Redirigiendo al dashboard...</p>
        </div>
      </div>
    );
  }

  // 3. Si no está cargando y NO hay usuario, mostrar la landing page.
  // Esta es la única condición en la que se debe renderizar la landing page.
  return (
          ...
          <div className="text-center">
            ...
            <div className="mt-10 flex flex-col items-center space-y-4 sm:flex-row sm:justify-center sm:space-y-0 sm:space-x-4">
              ...
              <Button
                ...
                className="border-white text-white hover:bg-white/20 hover:text-white transition-all duration-300 transform hover:scale-105 px-8 py-3 rounded-lg"
              >
                {/* Considera cambiar esto a una página de contacto o un modal */}
        ...
        <div className="container mx-auto px-4">
          <div className="grid gap-10 md:grid-cols-2 lg:grid-cols-4 mb-10">
            <div>
              <div className="flex items-center space-x-3 mb-4">
                <div className="relative h-10 w-10">
                    <Image
                      src="https://placehold.co/50x50/FFFFFF/0D9488?text=S" 
                      alt="SOFIA AI Medical Logo"
                      width={40}
                      height={40}
            ...
            <div>
              ...
              <ul className="space-y-2">
                ...
                <li><a href="/blog" className="hover:text-white text-teal-200 transition-colors">Blog</a></li>
                <li><a href="/faq" className="hover:text-white text-teal-200 transition-colors">Preguntas Frecuentes</a></li>
              </ul>
            </div>
          ...
          <div className="mt-12 border-t border-teal-700 pt-8 text-center">
            ...
            <p className="text-xs text-teal-400 mt-1">
              Diseñado con <HeartPulse size={14} className="inline text-teal-300 align-middle" /> para la comunidad médica.
            </p>
          </div>
        </div>
      </footer>
