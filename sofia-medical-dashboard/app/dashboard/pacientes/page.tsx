...
export default function PacientesPage() {
  ...
  const handleAddPatientSubmit = async () => {
    ...
    // Reset form
    setNewPatient({
      nombre: "", apellido: "", edad: "", genero: "", email: "", telefono: "", direccion: "",
    });
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">
            Administre la información y registros de sus pacientes.
          </p>
        </div>
            ...
            <div className="grid gap-4 py-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="space-y-2">
                  ...
                  <Input id="nombre" value={newPatient.nombre} onChange={handleInputChange} placeholder="Ingrese el nombre" required />
                </div>
                <div className="space-y-2">
                  <Label htmlFor="apellido">Apellido <span className="text-red-500">*</span></Label>
                  <Input id="apellido" value={newPatient.apellido} onChange={handleInputChange} placeholder="Ingrese el apellido" required />
                </div>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="space-y-2">
                  <Label htmlFor="edad">Edad</Label>
                  <Input id="edad" type="number" value={newPatient.edad} onChange={handleInputChange} placeholder="Ej: 30" />
                </div>
                <div className="space-y-2">
              ...
              <div className="space-y-2">
                ...
                <Input id="email" type="email" value={newPatient.email} onChange={handleInputChange} placeholder="paciente@ejemplo.com" />
              </div>
              <div className="space-y-2">
                <Label htmlFor="telefono">Teléfono</Label>
                <Input id="telefono" value={newPatient.telefono} onChange={handleInputChange} placeholder="Ej: 3001234567" />
              </div>
              <div className="space-y-2">
                <Label htmlFor="direccion">Dirección</Label>
                <Input id="direccion" value={newPatient.direccion} onChange={handleInputChange} placeholder="Ej: Calle 10 # 20-30" />
              </div>
            </div>
            <DialogFooter>
        ...
        <CardContent className="pt-6">
          ...
          <Tabs defaultValue="todos" className="space-y-4">
                   ...
                   <Table>
                     <TableHeader className="bg-gray-50">
                       <TableRow>
                         ...
                         <TableHead className="px-4 py-3 text-xs font-medium text-gray-500 uppercase tracking-wider">Edad</TableHead>
                         <TableHead className="px-4 py-3 text-xs font-medium text-gray-500 uppercase tracking-wider">Género</TableHead>
                         <TableHead className="px-4 py-3 text-xs font-medium text-gray-500 uppercase tracking-wider">Fecha Reg.</TableHead>
                     ...
                     <TableBody>
                       ...
                       {!loadingPacientes && !error && paginatedPacientes.map((paciente) => (
                         <TableRow key={paciente.id_usuario} className="hover:bg-gray-50">
                           ...
                           <TableCell className="px-4 py-3 text-gray-500 whitespace-nowrap">{paciente.fecha_registro ?? "N/A"}</TableCell>
                           <TableCell className="px-4 py-3 text-gray-500">
                             {paciente.ultimo_diagnostico ? (
                               <span className="text-sm">{paciente.ultimo_diagnostico}</span>
                             ) : (
                               <Badge variant="outline" className="border-amber-300 bg-amber-50 text-amber-800 font-normal text-xs py-0.5 px-2">
                                 Sin diagnóstico
                               </Badge>
                             )}
                           </TableCell>
                           ...
                           <TableCell className="px-4 py-3 text-right whitespace-nowrap">
                             <div className="flex justify-end items-center space-x-1">
                               ...
                               <Button variant="ghost" size="icon" className="text-gray-500 hover:text-blue-600 hover:bg-blue-100" title="Ver historial">
                                 <FileText className="h-4 w-4" />
                               </Button>
                               <Button variant="ghost" size="icon" className="text-gray-500 hover:text-yellow-600 hover:bg-yellow-100" title="Editar paciente">
                                 <Edit className="h-4 w-4" />
                               </Button>
                               <Button variant="ghost" size="icon" className="text-gray-500 hover:text-red-600 hover:bg-red-100" title="Eliminar paciente">
            ...
            <TabsContent value="recientes">
              <p className="text-center text-gray-500 py-10">Funcionalidad de pacientes recientes estará disponible pronto.</p>
            </TabsContent>
            <TabsContent value="sin-diagnostico">
              <p className="text-center text-gray-500 py-10">Funcionalidad de pacientes sin diagnóstico estará disponible pronto.</p>
            </TabsContent>
          </Tabs>
        </CardContent>
      </Card>
